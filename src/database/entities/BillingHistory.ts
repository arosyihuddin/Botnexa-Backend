...
export class BillingHistory extends BaseEntity {
    ...
    // Helper methods
    static async findByUserId(userId: number, limit: number = 10): Promise<BillingHistory[]> {
    ...
    static async findByDateRange(userId: number, startDate: Date, endDate: Date): Promise<BillingHistory[]> {
        return await BillingHistory.find({
            where: {
                user_id: userId,
    ...
    static async getTotalSpent(userId: number): Promise<number> {
